...
- `automaticStartTime` ([Time](#time) | "unset", optional): If not present, the trip's departure has not been detected or has not changed. If present, the new time that the train's departure from `startLocation` (or the existing `startLocation` of the trip) was detected. If set to `"unset"`, the trip was unmatched from an automatic departure that it was matched to prior.
- `automaticEndTime` ([Time](#time) | "unset", optional): If not present, the trip's arrival has not been detected or has not changed. If present, the new time that the train's arrival at `endLocation` (or the existing `endLocation` of the trip) was detected. If set to `"unset"`, the trip was unmatched from an automatic arrival that it was matched to prior.
